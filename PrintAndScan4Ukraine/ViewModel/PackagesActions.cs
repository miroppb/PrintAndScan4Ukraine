	...
	public partial class PackagesViewModel : INotifyPropertyChanged
	{
		...
		public async Task<bool> Save(Package package, int type = 0)
		{
			if (CanSave)
			{
				...
				{
					LastSaved = $"Last Saved {SelectedPackage.PackageId}: {DateTime.Now.ToShortDateString()} {DateTime.Now.ToLongTimeString()}";
		...
		private async void ShowHistory()
		{
			...
			if (historyWindow.DoubleClicked && historyWindow.SelectedPackageToUse != null)
			{
				...
				SelectedPackage.Sender_Name = p.Sender_Name; //in case we do a partial search
